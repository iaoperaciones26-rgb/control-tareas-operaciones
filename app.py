import streamlit as stimport pandas as pdimport gspreadfrom oauth2client.service_account import ServiceAccountCredentialsfrom datetime import datetimefrom st_aggrid import AgGrid, GridOptionsBuilder, GridUpdateMode
...
@st.cache_resourcedef conectar():client = gspread.authorize(creds)return client.open("BD_TAREAS_OPERACIONES")

try:spreadsheet = conectar()sheet = spreadsheet.worksheet("tareas")except Exception as e:st.error("❌ Error conectando con Google Sheets. Intenta recargar la app.")st.stop()
...
try:log_sheet = spreadsheet.worksheet("bitacora")except:log_sheet = spreadsheet.add_worksheet("bitacora", 100, 10)
...
@st.cache_data(ttl=60)def cargar_datos(refresh_key):return pd.DataFrame(sheet.get_all_records())
...
responsables_lista = ["Herman Jaramillo","Simon Gabela","Sandy Perez","Alexis Cevallos","Stalin Villalva","Andres Proaño","Clara Arteaga","Javier Ruiz","Julio Montenegro","Ivan Rodriguez","Martha Narvaez"]

estados = ["NUEVO","EN PROCESO","EN REVISION","REVISION FINAL","FINALIZADO"]
...
if "form" not in st.session_state:st.session_state["form"] = False

if "tarea_sel" not in st.session_state:st.session_state["tarea_sel"] = None
...
def calcular_avance(estado):return {"NUEVO":0,"EN PROCESO":25,"EN REVISION":50,"REVISION FINAL":75,"FINALIZADO":100}.get(estado,0)

def registrar_bitacora(id_tarea, accion):log_sheet.append_row([id_tarea,datetime.now().strftime("%Y-%m-%d %H:%M:%S"),accion])
...
with col1:if st.button("➕ Nueva tarea"):st.session_state["form"] = True

with col2:vista = st.radio("",["📋 Lista","📌 Kanban"],horizontal=True)
...
df = cargar_datos(st.session_state["refresh_key"])if df.empty:st.info("📭 No hay tareas registradas aún. Puedes crear una nueva tarea.")

df = calcular_tiempos(df)

if not df.empty:df["avance"] = df["estado"].apply(calcular_avance)
...
if st.session_state["form"]:with st.form("form_tarea"):
